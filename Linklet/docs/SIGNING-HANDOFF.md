...
Для обычного выпуска не использовать Organizer. После подготовки и отправки
тега выполнить из корня репозитория:
...
Публикация использует сохранённый профиль `notarytool` (по умолчанию
`scribe-notary`) и не передаёт пароль в командной строке. Для восстановления
после прерывания предусмотрен `scripts/publish-release.sh --resume`.
...
5. Запустить `scripts/release-preflight.sh`: он проверит тесты и полный архив
   исходников один раз и сохранит квитанцию для этого коммита.
6. Запустить `scripts/publish-release.sh`: он соберёт и подпишет universal архив,
   напрямую нотарифицирует и прикрепит ticket к приложению и DMG, подготовит
   Sparkle и опубликует GitHub Release.
7. Скрипт сам проверит два публичных assets и опубликует appcast последним.
   Сохранить `.xcarchive` из `build/releases/VERSION-BUILD/`; возвращать сборку
   владельцу для дополнительной подписи не требуется.
