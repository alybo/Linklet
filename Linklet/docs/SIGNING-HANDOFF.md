...
6. Убедиться, что ticket прикреплён (stapled), затем запустить `scripts/prepare-sparkle-update.sh` по инструкции обновлений: скрипт упакует и подпишет ZIP для Sparkle.
7. Опубликовать ZIP, соответствующие исходники и контрольные суммы в GitHub Releases. Затем обновить `updates/appcast.xml` в репозитории. Сообщить владельцу ссылку на релиз, Team ID и результат нотарификации; сохранить .xcarchive и dSYM. Возвращать ZIP владельцу для дополнительной подписи не требуется.
...
«Подготовь релиз Linklet из согласованных исходников: собери Release, подпиши Developer ID, пройди нотарификацию и прикрепи ticket. Затем подготовь ZIP и подпись Sparkle нашим скриптом, опубликуй ZIP с соответствующими исходниками в GitHub Releases и обнови appcast последним. Перед первым выпуском со Sparkle создай его ключ у себя и пришли только публичную часть, чтобы мы включили её в приложение. Сохрани .xcarchive и dSYM; сообщи ссылку на релиз, Team ID, номер сборки и SHA-256».
