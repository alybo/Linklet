...
Для текущего выпуска используй версию **1.2 (сборка 3)** и тег **v1.2**.
Сначала прочитай Linklet/docs/RELEASE-1.2.md, RELEASING.md, Linklet/docs/SIGNING-HANDOFF.md и
...
2. Публичная версия 0.1.1 со Sparkle уже выпущена. Используй существующий
   закрытый ключ издателя (account app.linklet.sparkle) и публичный ключ
   se2KhBLlhCaMRw3PEox9XshU29f+wlHRQTXQsvgVAF8= в Info.plist.
   Новый ключ не генерируй. Bundle identifier и Apple Developer Team сохраняй.

3. Проверь MARKETING_VERSION = 1.2 и CURRENT_PROJECT_VERSION = 3,
   выше опубликованной сборки 2. Эти значения уже записаны в проекте. Bundle identifier
